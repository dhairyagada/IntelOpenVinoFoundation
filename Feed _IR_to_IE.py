...
def get_args():
    ...
    parser.add_argument("-b", help=m_desc)
    args = parser.parse_args()

    return args


def load_to_IE(model_xml,model_bin):
    ### TODO: Load the Inference Engine API
    core = IECore(xml_config_file="")
    ...
    for key in net.layers.keys():
        if(not (key in layers_map)):
            print("Layer ",key," is Not Supported!")
            print("Check if Extension is found")
            exit()
    ### TODO: Load the network into the Inference Engine
    #load_ir = core.from_ir(model_xml,model_bin)
    core.load_network(net,"CPU")
